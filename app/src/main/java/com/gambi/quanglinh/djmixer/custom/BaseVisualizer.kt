...
import com.gambi.quanglinh.djmixer.utils.dpToPx
import com.gambi.quanglinh.djmixer.utils.getColorCompat
...
open class BaseVisualizer : View {
    ...
    protected var halfAbove = false
    protected var barDuration = 1000
    protected var barWidth = 1f
        set(value) {
            if (field > 0) {
                field = value
    ...
    private fun init() {
        ...
        this.loadedBarPrimeColor.color = context.getColorCompat(R.color.primary_main)
    ...
    private fun loadAttribute(context: Context, attrs: AttributeSet?) {
        ...
        try {
            ...
            maxAmp = typedArray.getFloat(R.styleable.iiVisu_maxAmp, 10f)
    ...
    private fun drawStraightBar(canvas: Canvas, startX: Float, height: Int, baseLine: Int) {
        ...
        var stopY =0f
        if(halfAbove){
            startY = baseLine + (height / 2).toFloat()
            stopY= baseLine.toFloat()
        }else
        {
            startY = baseLine.toFloat()
            stopY=  baseLine - (height / 2).toFloat()
        }
        if (startX <= width / 2) {
